...
use Zend_Loader_Autoloader as ZendAutoloader;

/**
 * Adhoc implementation of resource autoloading with PHP namespaces.
 * This class is untested and awaiting to get removed until the first zf2
 * stable release is up.
 *
 * Resource loader
 *
 * @uses       Zend_Loader
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class ResourceAutoloader extends \Zend_Loader_Autoloader_Resource
{
    /**
    ...
    public function __construct($options)
    {
        ...
        if (null !== $namespace) {
            ZendAutoloader::getInstance()->unshiftAutoloader($this, $namespace);
        } elseif (null !== $prefix) {
            if (!empty($prefix)) {
                $prefix .= '_';
            }
            ZendAutoloader::getInstance()->unshiftAutoloader($this, $prefix, true);
        }
    }
