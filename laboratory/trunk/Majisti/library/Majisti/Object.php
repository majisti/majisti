...
class Object implements Countable, Iterator
{
        ...
        if ( !empty($callbacks) ) {
            ...
            foreach ($this->_callbacks as $callback) {
                if ( !is_readable($callback) ) {
                    throw new Object\Exception('not a valid callback');
                }
            }
        } else {
            $callbacks = array();
     ...
     * @return Zend_Config
     */
    public function merge( Object $merge )
    {
        foreach ($merge as $key => $item) {
            if (array_key_exists($key, $this->_data)) {
                if ($item instanceof Object && $this->$key instanceof Object) {
                    $this->$key = $this->$key->merge(new Object($item->toArray()));
                } else {
                    $this->$key = $item;
                }
            } else {
                if ($item instanceof Object) {
                    $this->$key = new Object($item->toArray());
                } else {
                    $this->$key = $item;
                }
            }
